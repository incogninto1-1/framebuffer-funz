...
void genImage(){
	...
	for(;;){
		...
		for(int yOff=0; yOff<256; yOff++){
			for(int y = square.yPos; y< square.yPos+square.ySize; y++){
				for(int x=square.xPos; x < square.xPos+square.xSize; x++){
		...
		square.yPos = 355;
		for(int xOff=0; xOff<256; xOff++){
			for(int y = square.yPos; y< square.yPos+square.ySize; y++){
				for(int x=square.xPos; x < square.xPos+square.xSize; x++){
					XPutPixel(img, (x+xOff), y, 255);
				}
			}
			XPutImage(display, window, gc, img, 0, 0, 0, 0, width, height);
			blankFb();	
